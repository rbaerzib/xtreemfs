...
public class RPCNIOSocketClient extends LifeCycleThread {
    ...
    public RPCNIOSocketClient(SSLOptions sslOptions, int requestTimeout, int connectionTimeout, int sendBufferSize,
            int receiveBufferSize, SocketAddress localBindPoint) throws IOException {
        this(sslOptions, requestTimeout, connectionTimeout, sendBufferSize, receiveBufferSize, localBindPoint, "",
                false);
    }

    public RPCNIOSocketClient(SSLOptions sslOptions, int requestTimeout, int connectionTimeout, String threadName,
            boolean startAsDaemon) throws IOException {
        this(sslOptions, requestTimeout, connectionTimeout, -1, -1, null, threadName, startAsDaemon);
    }

    public RPCNIOSocketClient(SSLOptions sslOptions, int requestTimeout, int connectionTimeout, int sendBufferSize,
            int receiveBufferSize, SocketAddress localBindPoint, String threadName) throws IOException {
        this(sslOptions, requestTimeout, connectionTimeout, sendBufferSize, receiveBufferSize, localBindPoint,
                threadName, false);
    }

    public RPCNIOSocketClient(SSLOptions sslOptions, int requestTimeout, int connectionTimeout, int sendBufferSize,
            int receiveBufferSize, SocketAddress localBindPoint, String threadName, boolean startAsDaemon)
            throws IOException {
        super(threadName);
        setDaemon(startAsDaemon);
        if (requestTimeout >= connectionTimeout - TIMEOUT_GRANULARITY * 2) {
            throw new IllegalArgumentException("request timeout must be smaller than connection timeout less "
                    + TIMEOUT_GRANULARITY * 2 + "ms");
        }
        ...
        if (this.localBindPoint != null && Logging.isDebug()) {
            Logging.logMessage(Logging.LEVEL_DEBUG, Category.net, this,
                    "RPC Client '%s': Using the following address for outgoing connections: %s", threadName,
                    this.localBindPoint);
        }
    }

    public void sendRequest(InetSocketAddress server, Auth auth, UserCredentials uCred, int interface_id, int proc_id,
            Message message, ReusableBuffer data, RPCResponse response, boolean highPriority) {
        try {
    ...
    public void run() {
        ...
        /*
         * try { long now = System.currentTimeMillis(); int numKeys = selector.select(100); long duration =
         * System.currentTimeMillis()-now; if ((duration < 10) && (numKeys == 0)) {
         * Logging.logMessage(Logging.LEVEL_WARN, this,"detected broken select(int timeout)!"); brokenSelect = true; } }
         * catch (Throwable th) { Logging.logMessage(Logging.LEVEL_DEBUG,
         * this,"could not check Selector for broken select(int timeout): "+th); }
         */
        ...
        try {
            while (!quit) {
                ...
                try {
                    ...
                    Logging.logMessage(Logging.LEVEL_CRIT, this, OutputUtils.getThreadDump());
                }
            }
        } catch (Throwable thr) {
        ...
        if (con.canReconnect()) {
            ...
            try {
                ...
                if (Logging.isDebug()) {
                    ...
                    Logging.logMessage(Logging.LEVEL_DEBUG, Category.net, this, "socket send buffer size: %d", channel
                ...
                for (RPCClientRequest rq : con.getSendQueue()) {
                    rq.getResponse().requestFailed(
                            "sending RPC failed: server '" + con.getEndpointString() + "' not reachable (" + ex + ")");
                    rq.freeBuffers();
                }
                ...
                for (RPCClientRequest rq : con.getSendQueue()) {
                    rq.getResponse().requestFailed(
                            "sending RPC failed: reconnecting to the server '" + con.getEndpointString()
                                    + "' was blocked locally to avoid flooding");
                    rq.freeBuffers();
                }
                con.getSendQueue().clear();
                con.syncOwner = "";
                ...
                if (channel.doHandshake(key)) {
                    ...
                    while (true) {
                        ...
                        if (numBytesRead == -1) {
                            ...
                            closeConnection(key, "server (" + channel.socket().getRemoteSocketAddress().toString()
                                    + ") closed connection");
                            return;
                        }
                        if (buf.hasRemaining()) {
                        ...
                        case RECORD_MARKER: {
                            ...
                            if ((hdrLen <= 0) || (hdrLen >= RPCNIOSocketServer.MAX_FRAGMENT_SIZE) || (msgLen < 0)
                                    || (msgLen >= RPCNIOSocketServer.MAX_FRAGMENT_SIZE) || (dataLen < 0)
                                    || (dataLen >= RPCNIOSocketServer.MAX_FRAGMENT_SIZE)) {
                                Logging.logMessage(
                                        Logging.LEVEL_ERROR,
                                        Category.net,
                                        this,
                                        "invalid record marker size (%d/%d/%d) received, closing connection to client %s",
                                        hdrLen, msgLen, dataLen, channel.socket().getRemoteSocketAddress().toString());
                                closeConnection(key, "received invalid record marker from server ("
                                        + channel.socket().getRemoteSocketAddress().toString() + "), closed connection");
                                return;
                            }
    ...
    private void assembleResponse(SelectionKey key, RPCClientConnection con) throws IOException {
        ...
        try {
            ...
            if (rq == null) {
                ...
                Logging.logMessage(Logging.LEVEL_WARN, Category.net, this,
                        "received response for unknown request callId=%d", header.getCallId());
                return;
            }
    ...
    private void connectConnection(SelectionKey key) {
        ...
        try {
            ...
            con.connected();
            if (Logging.isDebug()) {
                Logging.logMessage(Logging.LEVEL_DEBUG, Category.net, this, "connected from %s to %s", con.getChannel()
                        .socket().getLocalSocketAddress().toString(), con.getEndpointString());
            }
        } catch (CancelledKeyException ex) {
            con.connectFailed();
    ...
    private void closeConnection(SelectionKey key, String errorMessage) {
        ...
        if (errorMessage != null && !errorMessage.isEmpty()) {
            Logging.logMessage(Logging.LEVEL_INFO, Category.net, this, "++++ Close Connection due to error: "
                    + errorMessage);
        }
        ...
        if (Logging.isDebug()) {
            Logging.logMessage(Logging.LEVEL_DEBUG, Category.net, this, "closing connection to %s",
                    con.getEndpointString());
        }
    }
                        ...
                        synchronized (con) {
                            ...
                            while (iter.hasNext()) {
                                ...
                                if (rq.getTimeQueued() + requestTimeout < now) {
                                    String logMessage = " ## Cur In Request size: " + con.getRequests().values().size();
                                    cancelRq.add(rq);
                                    iter.remove();
                                    ...
                                    try {
                                        logMessage += "\n ## Endpoint(in): " + con.getEndpointString();
                                    } catch (Exception e) {
                            ...
                            while (iter.hasNext()) {
                                final RPCClientRequest rq = iter.next();
                                if (rq.getTimeQueued() + requestTimeout < now) {
                                    String logMessage = "Cur Send Request size: " + con.getSendQueue().size();
                                    cancelRq.add(rq);
                                    iter.remove();
                                    logMessage += "\nCancel Send Request: " + rq.getClass();
                                    logMessage += "\n## Q-T vs now: " + rq.getTimeQueued() + " + " + requestTimeout
                                            + " vs " + now;
